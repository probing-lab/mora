...
def generate_data():
    ...
    for file, variable in files_vars:
        for power in [1, 2, 3]:
            print(f"Execute Mora on {file} with goal E({variable}**{power})")
            command = f'cd .. && python run.py --benchmarks benchmarks/{file} --goal {power} > oopsla-experiments/outputs/{file}-{power}.out'
            os.system(command)
    print()
