...
1. Open terminal in in the main 'mora/' folder.
2a. Run "python3.7 scripts/run_evaluation.py" to rerun Mora on subset programs shown in the tool demonstration paper submission.
...
4. You can run any single of the benchmarks using scripts in '/script' folder, e.g. "python3.7 scripts/run_benchmark_introA_2.py" for 'introA' program with goal '2'. Output will be stored as 'txt_NAME_TIMESTAMP' in the 'out' folder.
...
For example, one can run "mora("benchmarks/introA", goal=[1, "x^2", "x^3"]) to compute expected values of all variables and second and third moments of 'x' in program 'introA' and output to screen.
